...
worker.addEventListener('message', async(event: MessageEvent<WorkerMessage>) => {
  ...
  const files = _files.map((file) => {
    if (scale === 1){
      const canvas = new OffscreenCanvas(file.bitmap.width, file.bitmap.height);
      canvas.getContext("2d")?.drawImage(file.bitmap, 0, 0);
      return {...file, canvas};
    }
    const canvas = new OffscreenCanvas(file.bitmap.width * scale, file.bitmap.height * scale);
    canvas.getContext("2d")?.drawImage(file.bitmap, 0, 0, canvas.width, canvas.height);
    return {...file, canvas};
  });
