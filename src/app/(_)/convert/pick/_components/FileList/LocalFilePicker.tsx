...
export const LocalFilePicker = () => {
  ...
  const onChange = async(e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files) return;
    const processed = (await files2canvases(files)).map(({canvas, fileName}) => canvas2selectedFile(fileName, canvas));
  ...
  return (
    <>
      <input
        ...
        onChange={onChange}
      />
      <Button icon={<TfiHarddrive/>} onClick={onButtonClick}>Add File From Local</Button>
    </>
  );
}