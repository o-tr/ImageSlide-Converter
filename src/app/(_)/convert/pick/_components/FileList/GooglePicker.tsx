    ...
    const picker = new google.picker.PickerBuilder()
      ...
      .setCallback(async(data: { action: string; docs: any[] }) => {
        ...
        if (file.mimeType.startsWith("image/")) {
          ...
          const fileObject = new File([buffer], file.name, {type: file.mimeType});
          const canvas = (await files2canvases([fileObject])).map(({canvas, fileName}) => canvas2selectedFile(fileName, canvas));
